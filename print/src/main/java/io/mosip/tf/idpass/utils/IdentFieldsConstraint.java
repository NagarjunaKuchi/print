...
/**
 * The IdentFieldConstraint class is the input class to the Ident.Builder. The
 * getter methods to each field of interest should return the type expected by
 * Ident class. For example, 'gender' here is a String, but from the perspective
 * of usage as input into the 'Ident' builder class the gender is an int.
 *
 * The list of member fields in this class are the fields of interest to look
 * for.
 */
...
public class IdentFieldsConstraint {
	...
	/**
	 * These fields list names are the fields of interests to search for in the
	 * input json. Its corresponding field type is the type constraint for that
	 * field.
	 */
	...
	/**
	 * Additional pieces of information not in the protobuf message CardDetails
	 * definition, shall be stored as a key/value pair of extra information inside
	 * the QR code.
	 *
	 * The default visibility of these extra pieces of information is controlled by
	 * its associated boolean flag.
	 *
	 * Protobuf definitions:
	 * https://github.com/idpass/idpass-lite/blob/newprotofields/lib/src/proto/idpasslite.proto
	 *
	 * The extras below are fields in PostalAddress protobuf message, for example
	 * purposes only.
	 */
	...
	/**
	 * This maps a member fields of this class to an associated display string value
	 * in the map. For example, "Email" is the key and its value is from the "email"
	 * member field of this class instance.
	 *
	 * This is just to avoid the long repeated 'if' checks, when done manually.
	 */
	...
	/**
	 * The only constructor that accepts a map of found key/value pairs when the
	 * input json is traversed and using this class to provide the fields of
	 * interest.
	 * 
	 * @param m Is a map of found key/value pairs from an input json
	 */

	public IdentFieldsConstraint(Map<String, Object> m) throws IllegalAccessException, NoSuchFieldException {
		for (Map.Entry<String, Object> melem : m.entrySet()) {
			...
			try {
				...
				case "java.time.LocalDate":
					DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy/MM/d"); /// TODO: move to config? or
																							/// list of possible
																							/// combinations
	...
	/**
	 * Checks if the constraints defined in identfieldsconstraint.json is satisfied.
	 *
	 * These constraints are of the following: - presence/absence of a field value
	 * (in json) - type of the field value (in member fields) - total bytes count
	 * must fit QR code capacity (sum total during parsing)
	 *
	 * That is, some fields are required mandatory to be present. Some fields are
	 * optional. This presence/absence constraint is to be declared in the
	 * accompanying identfieldsconstraint.json file.
	 *
	 * The field value should render to its target type. This type constraint is
	 * according to the member field type within IdentFieldsConstraint class.
	 *
	...
	/**
	 * Returns the gender as numeric int for a particular language as set in the
	 * json configuration.
	 * 
	...
	public Ident.Builder newIdentBuilder() {
		...
		PostalAddress.Builder postalAddressBuilder = PostalAddress.newBuilder().setLanguageCode("en") /// TODO
		...
		if (fullName != null) {
			idb.setGivenName(fullName.split(" ")[0]);
			idb.setSurName(fullName.split(" ")[1]);
			idb.setGivenName(fullName);
		}
		if (lastName != null) {
		...
		if (dateOfBirth != null) {
			Date dobProto = Date.newBuilder().setYear(dateOfBirth.getYear()).setMonth(dateOfBirth.getMonthValue())
					.setDay(dateOfBirth.getDayOfMonth()).build();
